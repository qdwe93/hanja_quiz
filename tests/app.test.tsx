...
test("소리 재생 스위치는 기본값을 표시하고 설정을 저장한다", async () => {
  const user = userEvent.setup();
  render(<HanjaApp entries={entries} />);

  const musicSwitch = screen.getByRole("switch", { name: "노래재생" });
  const effectsSwitch = screen.getByRole("switch", { name: "효과음재생" });
  expect((musicSwitch as HTMLInputElement).checked).toBe(true);
  expect((effectsSwitch as HTMLInputElement).checked).toBe(true);

  await user.click(musicSwitch);
  await user.click(effectsSwitch);
  expect((musicSwitch as HTMLInputElement).checked).toBe(false);
  expect((effectsSwitch as HTMLInputElement).checked).toBe(false);

  const stored = JSON.parse(window.localStorage.getItem(PROGRESS_STORAGE_KEY) ?? "{}");
  expect(stored.audio).toEqual({ musicEnabled: false, effectsEnabled: false });
});

test("선택 세트의 25자 목록을 5×5로 보여 주고 한자와 음훈을 전환한다", async () => {
  const user = userEvent.setup();
  render(<HanjaApp entries={entries} />);

  expect(screen.getByText("한자 목록 (7급-1)")).toBeTruthy();
  const hanjaList = screen.getByLabelText("7급-1 한자 목록");
  expect(hanjaList.querySelectorAll("li")).toHaveLength(25);
  expect(hanjaList.textContent).toContain(entries[0].hanja);

  const toggle = screen.getByRole("button", { name: "漢字/음훈 전환, 현재 한자 보기" });
  await user.click(toggle);
  expect(toggle.getAttribute("aria-pressed")).toBe("true");
  const readingList = screen.getByLabelText("7급-1 음훈 목록");
  expect(readingList.querySelectorAll("li")).toHaveLength(25);
  expect(readingList.textContent).toContain(entries[0].eumhun);

  await user.click(screen.getByTestId("study-set-6급-3"));
  expect(screen.getByText("한자 목록 (6급-3)")).toBeTruthy();
  expect(screen.getByLabelText("6급-3 음훈 목록").querySelectorAll("li")).toHaveLength(25);
});
